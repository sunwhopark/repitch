...
export default function InboxPage() {
  ...
  // Deep link from a campaign creator: /dashboard/inbox?select=<id>.
  useEffect(() => {
  ...
  const banner = (selIsExcluded || (selDecision === "수락" && selOrigin)) ? (
    <div className="mb-4 space-y-2">
      ...
      {selDecision === "수락" && selOrigin && (
        <button
          type="button"
          onClick={() => router.push(`/dashboard/campaigns/${selOrigin}`)}
          ...
          {tabItems.map((item) => {
            ...
            return (
              <ListRow
                ...
                onManage={canManage ? () => router.push(`/dashboard/campaigns/${originCampaignId(item.proposal.id)}`) : undefined}
              />
            );
          })}
          {tabItems.length === 0 && (
