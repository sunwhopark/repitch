    ...
    <div className="h-full overflow-y-auto p-6 md:p-8">
      <div className="mx-auto w-full max-w-4xl">
        ...
        {status && (
          <div className="mt-4 flex items-center gap-2">
            ...
            <Link href="/demo/campaigns" className="text-xs text-muted-foreground underline hover:text-foreground">
              전체 보기
            </Link>
          </div>
        )}
        ...
        <div className="mt-8 space-y-10">
          {showActive && (
            <Section id="active" title="진행 중" items={active} highlightId={highlightId} onOpen={(id) => router.push(`/demo/campaigns/${id}`)} onEdit={startEdit} onDelete={startDelete} />
          )}
          {showEnded && (
            <Section id="ended" title="종료" items={ended} highlightId={highlightId} onOpen={(id) => router.push(`/demo/campaigns/${id}`)} onEdit={startEdit} onDelete={startDelete} />
          )}
        </div>
      </div>
      ...
      <CreateCampaignModal
        ...
        onSubmit={(c) => {
          addCampaign(c);
          setCreateOpen(false);
          if (status) router.push("/demo/campaigns"); // ensure the new card is visible
          setHighlightId(c.id);
        }}
      />
    </div>
