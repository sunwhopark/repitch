  ...
  return [
    {
      items: [
        ...
        { id: "home", title: "대시보드", icon: LayoutDashboard, href: "/demo" },
        {
          id: "inbox",
          title: "역제안 인박스",
          icon: Inbox,
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          icon: Megaphone,
          href: "/demo/campaigns",
          defaultOpen: pathname.startsWith("/demo/campaigns"),
          children: [
            { id: "c-active", title: "진행 중", icon: Hash, href: "/demo/campaigns?status=active" },
            { id: "c-ended", title: "종료", icon: Hash, href: "/demo/campaigns?status=ended" },
          ],
        },
      ],
    },
    {
      heading: "관리",
      items: [
        { id: "settlement", title: "정산", icon: Wallet, href: "/demo/settlement" },
      ],
    },
  ];
}
...
function activeIdFromPath(pathname: string, status: string | null): string {
  ...
  if (pathname.startsWith("/demo/inbox")) return "inbox";
  if (pathname.startsWith("/demo/influencers")) return "influencers";
  if (pathname.startsWith("/demo/products")) return "products";
  if (pathname.startsWith("/demo/campaigns")) {
    if (status === "active") return "c-active";
    if (status === "ended") return "c-ended";
    return "campaigns";
  }
  if (pathname.startsWith("/demo/settlement")) return "settlement";
  return "";
}
...
const ROUTE_BY_ID: Record<string, string> = {
  home: "/demo",
  inbox: "/demo/inbox",
  influencers: "/demo/influencers",
  products: "/demo/products",
  settlement: "/demo/settlement",
  campaigns: "/demo/campaigns",
  "c-active": "/demo/campaigns?status=active",
  "c-ended": "/demo/campaigns?status=ended",
};
...
function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-[100svh] w-full flex-col overflow-hidden bg-background text-foreground">
      {/* Demo bar — this route is the seed/demo build, open without auth. */}
      <div className="flex h-8 shrink-0 items-center justify-center gap-2 border-b border-border bg-foreground px-4 text-[12px] text-background">
        <span className="opacity-90">데모 버전이에요 · 실제 데이터가 아니에요</span>
        <a href="/login" className="font-semibold underline underline-offset-2 hover:opacity-80">
          실서비스 로그인
        </a>
      </div>
